...
## ステータス
⚠️ 設計制約あり・対応延期（2026-04-18）— 調査結果: GET /api/rooms/[code] は参加前の部屋確認（join flow）でも使用される。メンバーシップ強制はjoin flowを破壊するため実施不可。根本解決には「per-member招待トークン」のスキーマ変更が必要。現状は300req/minのレートリミットで列挙攻撃を緩和済み。
