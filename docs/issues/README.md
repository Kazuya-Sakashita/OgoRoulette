...
| ID | タイトル | 概要 | ステータス |
|----|--------|------|----------|
| [ISSUE-057](./issue-057-design-top-page-cta-hierarchy.md) | トップページの CTA 階層が不明確・ボタンテキストに不統一 | 4つのボタンが同等の重みで並び、ユーザーが迷う。「Xで続ける」のみ文体が異なりブランド一貫性を損なう | 🔴 未対応 |
| [ISSUE-059](./issue-059-design-play-page-info-density.md) | プレイページの情報密度が高く、ホイールまでのスクロールが必要 | ヘッダー・有効期限バナー・メンバー一覧・設定行がホイールの前に積み重なり、メインコンテンツが埋もれる | 🔴 未対応 |
...
| ID | タイトル | 概要 | ステータス |
|----|--------|------|----------|
| [ISSUE-061](./issue-061-design-join-page-engagement.md) | 招待参加ページが素っ気なく、参加へのモチベーションを高められていない | 参加人数とボタンのみのシンプルな構成。招待流入という最重要のユーザー獲得ポイントに演出がない | 🔴 未対応 |
| [ISSUE-062](./issue-062-design-expiry-banner-overwarning.md) | 有効期限バナーが通常時も警告色で表示され、ノイズになっている | 期限が十分残っていても amber 色バナーが常時表示。警告疲れを引き起こし、本当の警告が機能しなくなる | 🔴 未対応 |
| [ISSUE-063](./issue-063-design-home-spin-button-prominence.md) | ホームページの SPIN ボタンがルーレットと視覚的に分離しており、操作の関係が分かりにくい | ボタンとホイールの視覚的結びつきが弱い。「このボタンでホイールが回る」が直感的に伝わらない | 🔴 未対応 |
...
| ID | タイトル | 概要 | ステータス |
|----|--------|------|----------|
| [ISSUE-064](./issue-064-design-lobby-qr-code-visual.md) | ロビーページの QR コードと招待 UI が機能的で、ワクワク感に欠ける | QR コードが素の白四角。ロゴ埋め込み・参加時アニメーション・全員揃った時の演出がない | 🔴 未対応 |
| [ISSUE-065](./issue-065-design-login-page-provider-visual-weight.md) | ログインページのプロバイダーボタンに視覚的重み付けがなく、X が後から追加された印象 | 3ボタンが異なる色系統でバラバラ感。トップページとの実装重複も解消すべき | 🔴 未対応 |
